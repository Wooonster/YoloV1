...
class CNNBlock(nn.Module):
  def __init__(self, in_channels, out_channels, **kwargs):
    ...
    # other layers use the leaky rectified linear activation
...
class Yolov1(nn.Module):
  ...
  def _create_fcs(self, num_split, num_boxes, num_classes):
    S, B, C = num_split, num_boxes, num_classes
    return nn.Sequential(
        nn.Flatten(),
        nn.Linear(1024 * S * S, 4096),  # linear layer 1
        nn.Dropout(0.0),
        nn.LeakyReLU(0.1),
        nn.Linear(4096, S * S * (C + B * 5)) # linear layer 2
    )
